...
def logined_user(client):
    with client:
        ...
        client.post('/login', data={'email': 'test@email.com',
                                    'password': 'test',
                                    'remember': 'y'},
                    follow_redirects=True)
...
def test_index(client):
    ...
    assert b'Welcome to my homepage' in response.data
...
def test_user_register_login(client):
    with client:
        ...
        resp = client.post('/login', data={'email': 'test@email.com',
                                           'password': 'test',
                                           'remember': 'y'},
                           follow_redirects=True)
        assert resp.status_code == 200
...
def test_task_create(client, create_cat):
    ...
    assert "Task successfully created" in resp.get_data(as_text=True)
